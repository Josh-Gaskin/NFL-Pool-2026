...
function renderPinGate(errorMsg) {
  ...
  el.innerHTML = `
    <div class="pin-card">
      ...
      <p>Enter your 4-digit PIN. First time ever opening this tab? Whatever you type here becomes your PIN going forward.</p>
      <input type="password" inputmode="numeric" pattern="[0-9]*" maxlength="4" class="pin-input" id="pinInput" placeholder="••••">
      <button id="pinSubmitBtn">Continue</button>
  ...
  const submit = async () => {
    ...
    if (!/^\d{4}$/.test(pin)) {
      renderPinGate("PIN must be exactly 4 digits.");
      return;
    }
    const password = pinToPassword(pin);
    try {
      // Try signing in with this PIN first.
      await auth.signInWithEmailAndPassword(ADMIN_EMAIL, password);
      // onAuthStateChanged takes it from here.
    } catch (signInErr) {
      // Sign-in failed — either no PIN has ever been set, or this PIN is wrong.
      // Firebase's email-enumeration protection means we can't tell which
      // just from the sign-in error, so we try creating the account: if it
      // succeeds, no PIN existed yet and this one is now set. If it fails
      // because the account already exists, the PIN entered was wrong.
      try {
        await auth.createUserWithEmailAndPassword(ADMIN_EMAIL, password);
      } catch (signUpErr) {
        if (signUpErr.code === "auth/email-already-in-use") {
          renderPinGate("Incorrect PIN.");
        } else {
          renderPinGate(signUpErr.message);
        }
      }
    }
  };
  ...
  return `
    <div class="admin-card">
      ...
      <div class="row">
        ...
        <div class="field">
          <label for="assignWeek">Effective from week</label>
          <input type="number" id="assignWeek" min="1" max="22" value="1">
        </div>
        <div class="field" style="display:flex; align-items:flex-end;">
          <button id="assignBtn">Assign to player</button>
        </div>
      </div>
      <p class="tx-remaining">For the initial draft, leave this at week 1 no matter what the current week shows — that's what makes points from the start of the season count correctly.</p>
...
function attachAdminHandlers(players, week) {
  ...
  if (assignBtn) {
    assignBtn.addEventListener("click", async () => {
      ...
      const effectiveWeek = Number(document.getElementById("assignWeek").value) || 1;
      try {
        await db.collection("assignments").add({ team, player, effectiveWeek, createdAt: firebase.firestore.FieldValue.serverTimestamp() });
        await renderAdmin();
      } catch (err) {
